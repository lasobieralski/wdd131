//events.js
let tasks = []; //this is a variable

function taskTemplate(task) {
  return `
  <li ${task.completed ? 'class="strick"' : ""}>
    <p>${task.detail}</p>
    <div>
...
function newTask() {
  ...
  let task = document.querySelector("#todo").value; // I want the value the user enters
  // add it to our arrays tasks
  //let task = { "detail": input, "completed": false};

  tasks.push({ detail: task, completed: false });//that takes our tasks and renders out the input
  // render out the list
  renderTasks(tasks);
}
